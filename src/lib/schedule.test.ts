...
describe('buildGroupSchedule — Scenario 6 Variant B (3×4, special first 3 slots then normal)', () => {
  const groups: GroupDef[] = [1, 2, 3].map((n) => ({
    id: `G${n}`,
    label: `Gruppe ${n}`,
    slots: [`G${n}.1`, `G${n}.2`, `G${n}.3`, `G${n}.4`],
  }));
  const slotPlan: Array<[string, string]> = [
    ['G1', 'G1'],
    ['G2', 'G3'],
    ['G2', 'G3'],
    ['G1', 'G1'],
  ...
  const schedule = buildGroupSchedule(groups, '11:10', 11, AMERICANO, 3, slotPlan);

  // First 3 slots special (G1 alone, then G2+G3 parallel ×2), then normal.
  const expected: Array<[string, 1 | 2, string, string]> = [
    ['11:10', 1, 'G1.1', 'G1.4'],
    ['11:10', 2, 'G1.2', 'G1.3'],
    ['11:24', 1, 'G2.1', 'G2.4'],
